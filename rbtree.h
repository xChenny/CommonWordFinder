/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Andrew Chen and Robert Herley
 * Version     : 1.0
 * Date        :
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if (it != end()) {
            ...
            x = root_;              // Instantiate x and y
            y = NULL;
        }
        while (x != NULL) {         // Move x and y to correct pos.
          y = x;
          if (key < x->key()) {
            x = x->left();
          }
        ...
        }                           // set the new Node's parent to the correct
        z->set_parent(y);           // location
        if (y == NULL) {            // If y didn't iterate, it's empty, so z is
          root_ = z;                // the new root.
        }
        else if (z->key() == y->key()) {
          std::cout << "Warning: Attempt to insert duplicate key '"
                    << z->key() << "'." << std::endl;
          return;
        }
        else if (z->key() < y->key()) { // set z as the left child
          y->set_left(z);
        }
        else {                          // set z as the right right;
          y->set_right(z);
        }
        size_++;
        z->set_left(NULL);              // set default children and color.
        z->set_right(NULL);
        z->set_color(RED);
        insert_fixup(z);
    }
...
private:
    ...
    void insert_fixup(RedBlackNode<K, V> *z) {
        RedBlackNode<K, V> *y = new RedBlackNode<K, V>();
    ...
    /**
     * Returns the max of 3 arguements.
     */
    ...
    int diameter(Node<K, V> *node) const {
      ...
      return std::max(height(node->left()) + height(node->right()) + 2,
